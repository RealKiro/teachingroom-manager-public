...
### 2026-09-13 09:40:00 CST

- 事件：公开脱敏版新增设备报修模块，支持企业微信第三方报修应用（慧教云）数据接入。
- 分支：`main`
- 备注：新增 repair_requests 报修单表（报修人/联系方式/紧急程度/报修类型/负责人/报修地点/描述/状态流转 委托处理→已处理→已反馈/图片列表/台账自动匹配）；新增令牌鉴权的同步接入端点 `/api/integrations/huijiaoyun/repairs`（按平台单号去重，报修地点自动匹配台账条目）；前端新增"设备报修"弹窗（列表筛选、状态推进、管理员补录）。另提供面向学校信息中心/平台方的数据接口需求函模板（不入库）。不包含学校部署信息或运行数据。
- 验证：新增 repairs 测试 4 项（台账自动匹配、状态流转、同步去重、未登录拦截）全部通过。
