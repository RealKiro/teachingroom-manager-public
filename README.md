...
校园设备资产管理系统（TeachingRoom Manager）是一个轻量级 Web 系统，用于把教室设备 Excel 台账转成可持续维护、可审核、可备份的数据管理工具。
...
- 台账支持多类别：班级教室、教师、办公室，可按类别建立并登记设备台账。
- 内置字段覆盖鼠标/键盘（无线、有线）、加装音箱（立式、条式或自定义）、体温计及型号、维修日期/明细/保修期等；超级管理员可自定义扩展字段。
- 备件库存管理（名称、型号、存放位置、数量、自定义状态）与设备登记（使用人、使用地点、状态），支持设备转移并保留转移记录。
- 设备报修跟踪：支持从企业微信第三方报修应用（慧教云）同步报修单（Token 鉴权接入端点，按单号去重、自动匹配台账），也可手动补录；状态流转 委托处理 → 已处理 → 已反馈。
- 资产唯一编码 + 二维码标签打印，手机扫码免登录查询资产状态。
